...
class LLMClient:
    ...
    def inference_call(self, system_prompt: str, user_message: str) -> str:
        response = self.inference_llm.invoke([
    ...
    def _extract_content(content) -> str:
        """Normalize response content — handles str and list[dict] (Gemini)."""
    ...
    def inference_batch(self, system_prompt: str, user_messages: list[str]) -> list[str]:
        tasks = [self._acall_with_retry(self.inference_llm, system_prompt, msg) for msg in user_messages]
        return self._run_batch(tasks)
